...
export default {
  ...
  methods: {
    ...
    handleRemove (file) {
      // console.log(file)
      // 根据图片的临时路径，从pics数组中将对应的图片信息对象删除
    ...
    add () {
      this.$refs.addFormRef.validate(async valid => {
        ...
        // 执行添加的逻辑 1.收集表单数据 2.发送请求 3.提示用户 4.跳转到商品列表页面
        // lodash cloneDeep(obj) 深拷贝
        const form = _.cloneDeep(this.addForm)
        form.goods_cat = form.goods_cat.join(',')
        // 处理动态参数和静态属性
        this.manyTableData.forEach(item => {
          const newInfo = { attrId: item.attr_id, attr_value: item.attr_vals.join(' ') }
          this.addForm.attrs.push(newInfo)
        })
        this.onlyTableData.forEach(item => {
          const newInfo = { attrId: item.attr_id, attr_value: item.attr_vals }
          this.addForm.attrs.push(newInfo)
        })
